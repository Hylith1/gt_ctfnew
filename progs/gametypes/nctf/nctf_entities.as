...
class cFlagBase
{
    ...
    void touch( Entity @activator )
    {
        ...
        if ( this.owner.team == activator.team )
        {
            ...
            if ( CTF_CAPTURE_TIME.value > 0 && !ctfInstantFlag.boolean )
            {
                if ( this.captureTime < int( CTF_CAPTURE_TIME.value * 1000 ) )
                    return;
            }
        ...
        if ( CTF_UNLOCK_TIME.value > 0 && !ctfInstantFlag.boolean )
        {
            ...
            for( uint i = 0; i < inradius.size(); i++ )
			{
                ...
                if ( !tr.doTrace( origin, mins, maxs, center, target.entNum, MASK_SOLID ) && !isProtected(this.owner) )
                {
                    ...
                    if ( this.unlockTime > int( CTF_UNLOCK_TIME.value * 1000 ) )
                        this.unlockTime = int( CTF_UNLOCK_TIME.value * 1000 );
        ...
        if ( CTF_CAPTURE_TIME.value > 0 && !ctfInstantFlag.boolean )
        {
            ...
            for( uint i = 0; i < inradius.size(); i++ )
			{
                ...
                if ( !tr.doTrace( origin, mins, maxs, center, target.entNum, MASK_SOLID ) )
                {
                    ...
                    if ( this.captureTime > int( CTF_CAPTURE_TIME.value * 1000 ) )
                        this.captureTime = int( CTF_CAPTURE_TIME.value * 1000 );
